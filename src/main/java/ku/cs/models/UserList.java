...
public class UserList {
    ...
    public boolean changePassword(String username, String oldPassword, String newPassword) {
        User existedUsr = findUserByUsername(username);
        if(existedUsr != null) {
            if (existedUsr.validatePassword(oldPassword)) {
                existedUsr.setPassword(newPassword);
                return true;
            }
        }
        return false;
    ...
    public User login(String username, String password) {
        User existedUsr = findUserByUsername(username);
        if(existedUsr != null) {
            if (existedUsr.validatePassword(password)) {
                return existedUsr;
            }
        }
        return null;
